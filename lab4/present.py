...
pmt_inv = [0, 4, 8, 12, 16, 20, 24, 28, 32, 36, 40, 44, 48, 52, 56, 60,
...
def genRoundKeys(key):
    rounds = FULLROUND + 1
    roundKeys = {0: rounds, 1: key >> 16}
    
    for i in range(2, rounds + 1):
        # Rotate left
        key = rol(key, 61, 80)

        # Pass largest 4 bits to S box
        bits = key >> 76
        key -= bits << 76
        bits = sbox[bits]
        key += bits << 76
        ...
        # XOR bits 19 to 15
        bits = key & 0x000000000000000f8000
        key -= bits
        bits >>= 15
        bits ^= i-1
        key += bits << 15
...
def sBoxLayer(state, sbox):
    ...
    for i in range(len(nibbles)):
        nibble = nibbles.pop()
        sNibble = sbox[nibble]
        sNibbles.append(sNibble)

    sState = 0x0
    for i in range(len(sNibbles)):
        sState += sNibbles.pop() << (i*4)
    
    return sState

def pLayer(state, pmt):
    bitarray = []
    temp = []
    for i in range(64):
        bit = state & 0b1
...
def present_round(state, roundKey):
    ...
    state = pLayer(state, pmt)
    return state

def present_inv_round(state, roundKey):
    state = pLayer(state, pmt_inv)
